...
const fetchData = async () => {
  ...
  if (breaksError) {
    await Swal.fire("Error", "Error fetching breaks: " + breaksError.message, "error");
    setLoading(false);
    return;
  }
  ...
  return (
     <div className={`w-full flex flex-col flex-grow min-h-0 px-4 sm:px-6 lg:px-8 py-4 ${
    theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'
      ...
      <div className="flex justify-between items-center mb-4">
       ...
       <div className={`rounded-lg shadow overflow-hidden flex-grow mb-6
       lg:w-[1210px] 
        ${
      theme === 'dark' ? 'bg-gray-800' : 'bg-white'
    } transition-colors duration-200`}>
      <h2 className={`text-lg md:text-xl font-bold px-4 py-3 ${
      ...
      <div className={`rounded-lg shadow overflow-hidden flex-grow ${
      theme === 'dark' ? 'bg-gray-800' : 'bg-white'
    } transition-colors duration-200`}>
      <h2 className={`text-lg md:text-xl font-bold px-4 py-3 ${
        theme === 'dark' ? 'bg-gray-700 text-white' : 'bg-gray-50'
